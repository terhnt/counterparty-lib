...
from unopartylib.lib.evm import opcodes, ethutils, specials, vm
from unopartylib.lib.evm.vm import VmExtBase
from unopartylib.lib.evm.exceptions import *
from unopartylib.lib.evm.blocks import Log
import logging
...
from unopartylib.lib.evm import exceptions
from unopartylib.lib.evm.address import Address, mk_contract_address, unique_address_list
...
def apply_transaction(db, block, tx):
    ...
    log_tx.debug('TX NEW', **tx.log_dict())
    ...
    suicides = unique_address_list(block.suicides)
    block.suicides = []
    for s in suicides:
        block.del_account(s)
    block.logs = []
