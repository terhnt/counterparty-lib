...
def cancel_rps (db, rps, status, block_index):
    ...
    util.credit(db, rps['source'], 'XUP', rps['wager'], action='recredit wager', event=rps['tx_hash'])
...
def update_rps_match_status (db, rps_match, status, block_index):
    ...
    if status in ['expired', 'concluded: tie']:
        ...
        util.credit(db, rps_match['tx0_address'], 'XUP',
                    rps_match['wager'], action='recredit wager', event=rps_match['id'])
        # Recredit tx1 address.
        util.credit(db, rps_match['tx1_address'], 'XUP',
                    rps_match['wager'], action='recredit wager', event=rps_match['id'])
...
def parse(db, tx, message):
    ...
    if status == 'open':
        ...
        rps_parse_cursor.execute('''SELECT * FROM balances \
                                    WHERE (address = ? AND asset = ?)''', (tx['source'], 'XUP'))
    ...
    if status == 'open':
        util.debit(db, tx['source'], 'XUP', wager, action="open RPS", event=tx['tx_hash'])
