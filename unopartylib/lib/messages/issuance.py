...
from unopartylib.lib import (config, util, exceptions, util)
...
def validate (db, source, destination, asset, quantity, divisible, callable_, call_date, call_price, description, block_index):
    ...
    if asset in (config.MAINCOIN, config.TOKEN):
        problems.append('cannot issue {} or {}'.format(config.MAINCOIN, config.TOKEN))
    ...
    if quantity or (block_index >= 315000 or config.TESTNET):   # Protocol change.
        if not reissuance or (block_index < 310000 and not config.TESTNET):  # Pay fee only upon first issuance. (Protocol change.)
            ...
            cursor.execute('''SELECT * FROM balances \
                              WHERE (address = ? AND asset = ?)''', (source, config.TOKEN))
...
def parse (db, tx, message):
    ...
    # Debit fee.
    if status == 'valid':
        util.debit(db, tx['source'], config.TOKEN, fee, action="issuance fee", event=tx['tx_hash'])
