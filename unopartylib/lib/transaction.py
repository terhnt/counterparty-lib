...
from unopartylib.lib.transaction_helper import serializer, p2sh_encoding
...
def construct_coin_selection(encoding, data_array, source, allow_unconfirmed_inputs, unspent_tx_hash, custom_inputs,
    ...
    if not sufficient_funds:
        ...
        raise exceptions.BalanceError('Insufficient {} at address {}, currently has {}. (Need approximately {} {}.) To spend unconfirmed coins, use the flag `--unconfirmed`. (Unconfirmed coins cannot be spent from multi‐sig addresses.)'.format(config.BTC, source, btc_in, total_btc_out / config.UNIT, config.BTC))
