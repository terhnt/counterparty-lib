...
from unopartylib.lib import util, backend
...
def test_search_raw_transactions_output():
    ...
    assert tx['vout'][0]['scriptPubKey']['addresses'] == ['Ukn3L4dgG13R3dSdxLvAAJizeiaW7cyUFz',
                                          'UXJo7MANGsCfJM8LwRdkRsWWBxw6Tdshxt',
                                          'UZ1XAmDF6Br2JKxahJWhuHzGawK7zxPtYW']
    ...
    assert tx['vout'][1]['scriptPubKey']['addresses'] == ['UZ1XAmDF6Br2JKxahJWhuHzGawK7zxPtYW']
...
def test_search_raw_transactions_unconfirmed(server_db):
    ...
    # create send
    v = int(100 * 1e8)
    send1hex = util.api('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': 'XUP', 'quantity': v})
    ...
    # create send
    v = int(100 * 1e8)
    send2hex = util.api('create_send', {'source': ADDR[0], 'destination': ADDR[1], 'asset': 'XUP', 'quantity': v})
