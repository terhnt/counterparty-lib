...
from unopartylib.lib import (config, util, blocks, check, backend, database, transaction, exceptions)
from unopartylib.lib.backend.indexd import extract_addresses, extract_addresses_from_txlist

from unopartylib.test.fixtures.params import DEFAULT_PARAMS as DP
from unopartylib.test.fixtures.scenarios import UNITTEST_FIXTURE, INTEGRATION_SCENARIOS, standard_scenarios_params
...
def getrawtransaction(db, txid, verbose=False):
    ...
    When verbose=True we mock the unobtaniumd RPC response, it's incomplete atm and only mocks what we need
    ...
    for tx in scenario:
        if tx[0] != 'create_next_block':
            ...
            with MockProtocolChangesContext(**(mock_protocol_changes or {})):
                module = sys.modules['unopartylib.lib.messages.{}'.format(tx[0])]
...
def check_outputs(tx_name, method, inputs, outputs, error, records, comment, mock_protocol_changes, server_db):
    ...
    try:
        tested_module = sys.modules['unopartylib.lib.{}'.format(tx_name)]
    except KeyError:    # TODO: hack
        tested_module = sys.modules['unopartylib.lib.messages.{}'.format(tx_name)]
...
class MockProtocolChangesContext(object):
    def __init__(self, **kwargs):
        from unopartylib.test.conftest import MOCK_PROTOCOL_CHANGES
